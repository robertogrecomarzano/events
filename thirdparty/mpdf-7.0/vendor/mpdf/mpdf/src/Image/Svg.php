...
class Svg
{
    ...
    public function __construct(Mpdf $mpdf, Otl $otl, CssManager $cssManager, SizeConverter $sizeConverter, ColorConverter $colorConverter, LanguageToFontInterface $languageToFont, ScriptToLanguageInterface $scriptToLanguage)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
    ...
    function svgStyle($critere_style, $attribs, $element)
    {
        ...
        if ($critere_style['stroke'] != 'none') {
            ...
            if (isset($critere_style['stroke-miterlimit'])) {
                if ($critere_style['stroke-miterlimit'] == 'none') {} else if (preg_match('/^[\d.]+$/', $critere_style['stroke-miterlimit'])) {
    ...
    function svgDefineTxtStyle($critere_style)
    {
        ...
        if (isset($critere_style['font-family']) && $critere_style['font-family'] != 'inherit') {
            ...
            foreach ($aux_fontlist as $f) {
                ...
                if ((! $this->mpdf->usingCoreFont && in_array($v, $this->mpdf->available_unifonts)) || ($this->mpdf->usingCoreFont && in_array($v, [
                    'courier',
                    'times',
                    'helvetica',
                    'arial'
                    ...
                    'big5',
                    'gb'
                ]) || isset($this->svg_font[$v][$svgfontstyle])) { // mPDF 6
    ...
    function ImageSVG($data)
    {
        ...
        xml_set_element_handler($svg2pdf_xml_parser, [
            $this,
            'xml_svg2pdf_start'
        ], [
            $this,
        ...
        xml_set_character_data_handler($svg2pdf_xml_parser, [
            $this,
            'characterData'
        ]);
