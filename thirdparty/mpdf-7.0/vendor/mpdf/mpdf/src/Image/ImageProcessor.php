...
class ImageProcessor
{
    ...
    public function __construct(Mpdf $mpdf, Otl $otl, CssManager $cssManager, SizeConverter $sizeConverter, ColorConverter $colorConverter, ColorModeConverter $colorModeConverter, Cache $cache, LanguageToFontInterface $languageToFont, ScriptToLanguageInterface $scriptToLanguage, LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
    ...
    public function getImage(&$file, $firsttime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
    {
        ...
        if (empty($data)) {
            ...
            if ($orig_srcpath && $this->mpdf->basepathIsLocal && $check = @fopen($orig_srcpath, "rb")) {
                fclose($check);
                $file = $orig_srcpath;
                $this->logger->debug(sprintf('Fetching (file_get_contents) content of file "%s" with local basepath', $file), [
            ...
            if (! $data && $check = @fopen($file, "rb")) {
                fclose($check);
                $this->logger->debug(sprintf('Fetching (file_get_contents) content of file "%s" with non-local basepath', $file), [
                    'context' => LogContext::REMOTE_CONTENT
                ]);
                $data = file_get_contents($file);
                $type = $this->guesser->guess($data);
                        ...
                        if ($colspace == 'Indexed') { // generate Alpha channel values from tRNS
                            ...
                            if ($p) {
                                ...
                                if ($colspace == 'DeviceGray') { // ct===0
                                    $trns = [
                                        $this->translateValue(substr($t, 0, 2), $bpc)
                                    ];
                ...
                if ($p) {
                    ...
                    if ($colspace == 'DeviceGray') { // ct===0
                        ...
                        if ($targetcs == 'DeviceCMYK') {
                            $col = $this->colorModeConverter->rgb2cmyk([
                                3,
                                $trns[0],
                                $trns[1],
                                $trns[2]
    ...
    private function imageError($file, $firsttime, $msg)
    {
        ...
        $this->logger->warning(sprintf('%s (%s)', $msg, $file), [
            'context' => LogContext::IMAGES
        ]);
    }
