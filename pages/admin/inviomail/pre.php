...
switch ($action) {
    case "confirm":
        foreach ($_POST["event"] as $id_evento => $type) {
            Database::query("UPDATE eventi SET inviare=1, inviare_template=? WHERE id_evento=?", [
                $type,
                $id_evento
            ]);
            if ($_POST["reset"])
                Database::query("UPDATE utenti SET is_reminder_inviato=0, ts_reminder_inviato=NULL, is_link_inviato=0, ts_link_inviato=NULL WHERE id_evento=? AND record_attivo=1", [
                    $id_evento
                ]);
        }
