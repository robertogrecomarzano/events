...
class Captcha extends Plugin
{
    ...
    {
        if (! empty($_REQUEST['captcha_value'])) {
            if (empty($_SESSION['captcha']) || trim(strtolower($_REQUEST['captcha_value'])) != $_SESSION['captcha']) {
                return false;
            } else {
                return true;
            }
    ...
    {
        $a = new Captcha();
        $this->resourcesPath = $a->serverFolder() . DS . "res";
        $this->CreateImage();
        echo json_encode($a->webFolder() . "/res/captcha.jpeg");
    }
    ...
    function Draw()
    {
        ...
        $imgArgs = array(
            "src" => $a->webFolder() . "/res/captcha.jpeg?a=$ini",
            "border" => 1,
            "id" => "captcha",
            "style" => "vertical-align:middle"
        );
        $img = HTML::tag("img", $imgArgs);

        $linkArgs = array(
            "href" => "javascript:reloadCaptcha()",
            "title"=>Language::get("REFRESH")
        );
        $link = HTML::tag("a", $linkArgs, "<span class='fas fa-sync-alt text-success'></span>");

        $txt = Form::textbox(array(
            "id" => "captcha_value",
            "name" => "captcha_value",
            "placeholder" => Language::get("CAPTCHA_CODE"),
            "style" => "display:inline;",
            "required" => "",
            "oninvalid" => "this.setCustomValidity('{form_lang value='CAPTCHA_REQUIRED'}')"
        ), null);
        return "<div class=\"form-group\">$link$img</div><div class=\"form-group input-group\"><span class=\"input-group-addon\"><i class='fas fa-user-shield'></i></span>$txt</div>";
    }
    ...
    protected function WriteImage()
    {
        if ($this->imageFormat == 'png' && function_exists('imagepng')) {
            ...
            imagejpeg($this->im, $this->resourcesPath . DS . "captcha.jpeg", 80);
        }
    }
